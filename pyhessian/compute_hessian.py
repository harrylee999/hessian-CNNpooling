...
def precomputed_plot(args):
    ...
    z = np.load("D:\code\Python\myfile\smart_computer\pyhessian\logs\hessian\{}/z.npy".format(args.model_dir.split('.')[1][6:]))
    ax = plt.axes(projection='3d')
    MAX_Z = 10
    ax.set_zlim(0, MAX_Z)
    ...
    for ii in range(0,360,30):
        ax.view_init(elev=10., azim=ii)
        plt.savefig("D:\code\Python\myfile\smart_computer/pyhessian/logs/hessian/{}/pdf/angle{}.pdf".format(args.model_dir.split('.')[1][6:],ii))
